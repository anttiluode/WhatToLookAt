# Vendored/adapted from Hugging Face diffusers community example
# examples/community/masked_stable_diffusion_xl_img2img.py (Apache-2.0).
# Local change: mask-noise uses a separate generator so full-mask parity does
# not perturb the main scheduler RNG; zero masks are handled safely.
...
class MaskedStableDiffusionXLImg2ImgPipeline(StableDiffusionXLImg2ImgPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        mask_noise_generator = torch.Generator(device=device).manual_seed(int(mask_noise_seed))
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise = randn_tensor(
                    shape,
                    generator=mask_noise_generator,
                    device=device,
                    dtype=latents.dtype,
                )
    ...
    def blur_mask(self, pil_mask, blur):
        ...
        peak = float(mask_blur.max())
        if peak <= 0:
            return torch.zeros((*mask_blur.shape, 3), dtype=torch.float32)
        return torch.from_numpy(np.tile(mask_blur / peak, (3, 1, 1)).transpose(1, 2, 0))
